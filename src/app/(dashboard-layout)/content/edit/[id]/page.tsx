...
export default function EditContentPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/content/update/${id}`, {
                method: 'PUT',
            ...
            if (result.success) {
                dispatch(showToast({ message: 'Content updated successfully', type: 'success' }));
                router.push('/content');
            } else {
                dispatch(showToast({ message: result.message || 'Failed to update content', type: 'error' }));
            }
        } catch (error) {
            console.error('Error updating content:', error);
            dispatch(showToast({ message: 'Error updating content', type: 'error' }));
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Edit Content</h1>
                        <p className="mt-1 text-gray-500 dark:text-gray-400">Update the details of your content module.</p>
                    </div>
                </div>
            </div>
